...
# ---------------------------------------------------------------------------
# SQLite browser -- /admin/sqlite  and  /admin/sqlite/<table>
# ---------------------------------------------------------------------------

SQLITE_PAGE_SIZE = 100


@admin_bp.route("/sqlite")
def sqlite_table_list():
    ...
    conn.close()
    return render_template("admin/sqlite_table_list.html", tables=tables)


@admin_bp.route("/sqlite/<table>")
def sqlite_table_view(table):
    """Paginated, searchable browser for any SQLite table."""
    ...
    if q:
        ...
        rows_raw = cur.execute(
            ...
            lp + [SQLITE_PAGE_SIZE, offset],
        ).fetchall()
    else:
        total = cur.execute(f'SELECT COUNT(*) FROM "{table}"').fetchone()[0]
        offset = (page - 1) * SQLITE_PAGE_SIZE
        rows_raw = cur.execute(
            f'SELECT * FROM "{table}" ORDER BY "{sort_col}" {order} LIMIT ? OFFSET ?',
            [SQLITE_PAGE_SIZE, offset],
        ).fetchall()
    conn.close()
