...
from models import (Organization, OrganizationCompletionsDemographics,
                    OrganizationDemographics, Program, ProgramDemographics, db)
...
def load_organization_demographics(session: Session, year: str) -> int:
    ...
    session.query(OrganizationDemographics).filter(
        OrganizationDemographics.org_id.in_(org_dict.values())
...
def load_program_demographics(session: Session, year: str) -> int:
    ...
    programs = (
        session.query(Program).filter(Program.org_id.in_(org_dict.values())).all()
    )
    ...
    session.query(ProgramDemographics).filter(
        ProgramDemographics.program_id.in_(prog_map.values())
    ).delete(synchronize_session=False)
    session.query(OrganizationCompletionsDemographics).filter(
        OrganizationCompletionsDemographics.org_id.in_(org_dict.values())
    ).delete(synchronize_session=False)
