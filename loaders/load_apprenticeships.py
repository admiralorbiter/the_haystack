...
from models import OrgAlias, Organization, OrgContact, RegionCounty, db

DATA_FILE = (
    PROJECT_ROOT / "data" / "raw" / "apprenticeship" / "partner-finder-listings.csv"
)
...
def load_apprenticeships():
    ...
    with app.app_context():
        ...
        print(
            f"Loaded {len(existing_training_orgs)} existing training providers for fuzzy matching."
        )
        ...
        kc_counties = (
            db.session.query(RegionCounty.county_fips)
            .filter_by(region_id="kc-msa")
            .all()
        )
        ...
        for _, row in df.iterrows():
            ...
            def _clean_contact(val):
                ...
                if (
                    not v_low
                    or v_low == "nan"
                    or "does not apply" in v_low
                    or v_low == "undefined"
                    or v_low == "none@none.com"
                ):
                    return None
                return val
            ...
            if match_id:
                ...
                if contact_person or contact_email or contact_phone:
                    existing_contact = (
                        db.session.query(OrgContact)
                        .filter_by(
                            org_id=match_id, contact_role="Apprenticeship Partner"
                        )
                        .first()
                    )
        ...
        print(
            f"Apprenticeship Load Complete: {added} new organizations added, {merged} merged with existing."
        )
